...
class RadianceFieldPRBRT(mi.python.ad.integrators.common.RBIntegrator):
    ...
    def sample(self, mode, scene, sampler,
        ...
        L = mi.Spectrum(0.0 if primal else state_in)
        dL = mi.Spectrum(dL if dL is not None else 0)
        ...
        # Reservoir sampling for DRT (differential ratio tracking)
        Tr = mi.Float(1.0)
        w_acc = mi.Float(0.0)
        reservoir = (mi.Float(0.0), mi.Float(0.0))
        
        while active:
            # Sample next interaction distance using majorant
            # -log(1 - ξ) / σ_majorant where ξ is uniform random
            u = dr.clip(sampler.next_1d(active), 1e-6, 1.0 - 1e-6)
            ...
            # u_reservoir = dr.clip(sampler.next_1d(active), 1e-6, 1.0 - 1e-6)
            # should_update_reservoir = (u_reservoir * w_acc) < w_step
            ...
            with dr.resume_grad(when=not primal):
                ...
                # Russian roulette: only interact with probability interaction_prob
                    ...
                    if self.stopgrad_density:
                        ...
                        dr.backward_from(
                            ...
                            (
                                ...
                                + Le
                                - trans_grad_buffer * dr.detach(L) * dr.detach(interaction_mask)
                            )
                        )

            # Update transmittance gradient buffer
            trans_grad_buffer *= (1.0 - dr.detach(interaction_mask))
            
            # Stop if throughput becomes too small
            active &= dr.any(Tr > self.min_throughput)
        
        # # DRT gradient update
        # if not primal and not self.stopgrad_density:
        #     t, dt = reservoir
        #     u = dr.clip(sampler.next_1d(active == active), 1e-6, 1.0 - 1e-6)
        #     t += u * dt
        ...
        #     dr.backward_from(dL * (sigmat * dr.detach(Le)))

        # Return results
        return L if primal else dL, mi.Bool(True), [], L
