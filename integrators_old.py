...
class RadianceFieldPRBRT(mi.python.ad.integrators.common.RBIntegrator):
    ...
    def sample(self, mode, scene, sampler,
        ...
        while active:
            ...
            ξ = dr.clip(sampler.next_1d(active), 1e-6, 1.0 - 1e-6)

            # Get current majorant value at current position
            p = ray(t)
            majorant = self.majorant_grid.eval(dr.clip(p, 0.0, 1.0))[0]
            dt = dr.maximum(-dr.log(1.0 - ξ) / majorant, self.min_step_size)
            t += dt
            num_steps += 1
            ...
            with dr.resume_grad(when=not primal):
                ...
                # Vectorized emission calculation: only emit when should_interact is true
                if should_interact:
            ...
            active &= dr.any(β > self.min_throughput)
